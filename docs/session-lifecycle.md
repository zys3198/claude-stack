...
记录的时间戳只认数值。缺字段或写错类型的记录排在排序前面，不会成为「上次会话」的取值，计数照常、明细跳过。
...
机制对所有 git 仓库一视同仁，`~/.claude` 不做特殊跳过。它本身是个 git 仓库，在里面开会话时会正常报出主检出的未提交改动。它的工作树按同一规则建在 `.claude/worktrees/` 下，该目录已被忽略，工作树里的内容不会混进仓库状态。
