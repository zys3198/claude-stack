...
本文件记录 `skill-auditor` 的演进。格式参考 [Keep a Changelog](https://keepachangelog.com/)，日期 YYYY-MM-DD。

## [v2.4.0] - 2026-09-22

### Added

- 新增「减量审计」模式。用户要求缩小单个 Skill 且保持触发、执行、门禁和验收效果时，根入口必须读取 `references/skill-size-optimization.md`。
- 新参考文件把固定注入、description、调用正文和回流材料分开测量，并给出信息层级、分支测试、context pointer 合同、原文迁移、逐句剪除和行为等效门禁。
- `test-prompts.json` 增加根入口减量场景；新增 Claude CLI 可发现的 `evals/size-optimization/case.yaml`，检查减量模式、信息层级、baseline 和 holdout。

### Changed

- `skill-trimmer-by-user` 的「需工程修复」结果增加转介：能力应保留但根入口出现 `sprawl` 时，交由 `/skill-auditor-by-user` 做减量审计；库级 Skill 不复制方法正文。

### Evidence

- Matt Pocock `mattpocock-skills@1.2.3`：`writing-for-agents/SKILL.md` 与 `SKILL-MECHANICS.md` 的 information hierarchy、context pointer、progressive disclosure、sprawl、environment、no-op 和 invocation load。
- Claude Code 官方 Skill 文档：description、正文和 `disable-model-invocation` 的实际加载合同。
- 本机 2026-09-21 的 21 个自建 Skill 精简记录：根入口实际减量后需要一套可重复审计、同条件验证且不削弱门禁的方法。

## [v2.3.1] - 2026-09-21

### Removed

- 文件尾部 HTML 注释。它记录的「增删检查项、路由表和设计规则必须引用真实事故、官方变更或失败测试」与 `references/MAINTENANCE.md` 的「变更门禁」是同一句话，版本沿革由本文件承担。

### Evals（资产更新，不 bump 版本）

- 5 个 v1alpha1 case 迁为 CLI 2.1.278 可发现的 `evals/<case>/case.yaml`，保留原 prompt、Sonnet 模型和验收词，grader 改为确定性 regex。全量 5/5 通过，费用 0.27 美元。
- 旧 `evals/eval.yaml` 与 `evals/cases/*.yaml` 已在 `~/.claude/backups/skills-before-slim-2026-09-21/` 核实存在；删除被权限分类器拒绝，现仍保留但不被当前 runner 发现。

## [v2.3.0] - 2026-09-21

### Added

- 核心边界新增一条：按「谁能调用」分两类——只能手动敲的（`disable-model-invocation: true`）负责编排，能被模型自动选中的负责执行；编排可以调用执行，编排之间不互相调用。理由是两个编排互相调用会让同一个功能出现两块入口，各自维护一份会漂移的流程。

### Evidence

- Matt Pocock `mattpocock/skills` 仓库 README（<https://github.com/mattpocock/skills>）：skill 集合按 user-invoked / model-invoked 一条轴划分，治理规则原文为 user-invoked 可以调用 model-invoked，不能调用另一个 user-invoked。

### 备注

- 本机已有 `disable-model-invocation` 的用法（`eli5`），缺的是编排层的调用约束，此前未落盘。

## [v2.2.0] - 2026-09-21

### Added

- 核心边界新增两条：只读某份材料取词汇或事实（读 `CONTEXT.md` 取术语）是一行指针，任何 skill 都能带，**改变**那份材料才构成一个 skill；skill 之间的依赖写成 `/skill` 形式的散文调用，不跨目录链 `../other-skill/FILE.md`，共享材料留在拥有它的那个 skill 内部。
- 「通用 Skill 原则」新增第 7 条「不加冗余机制」：自然语言入口已经能表达的控制方式（校验、收窄范围、只检查不改动）不再加 flag、计数器或兄弟 skill，加了会把一个功能拆成两块表面并多出一份互相漂移的维护成本；给一个症状加限制之前先分清「按设计正常工作」与「质量缺陷（修 prompt）」两种来源。
- Router 追加检查新增第 8 条「同步义务」：skill 集合或它在流程里的位置发生变化时，router 必须跟着更新，漏掉一个它从不提起的新 skill 或还路由到一个已经不做那件事的旧 skill，等于一条会骗人的路由；路由表里每个 skill 只描述自己那一跳的位置，整张图由 router 拥有。

### Evidence

- Matt Pocock `mattpocock-skills@1.2.3` 插件 payload 一手文件：`.agents/invocation.md:16-20`（依赖用 prose 调用、被动读与主动纪律的分界）、`.agents/adr/0001-explicit-setup-pointer-only-for-hard-dependencies.md`、`.agents/writing-docs.md:64-69`（节点与 router 的分工）、仓库根 `CLAUDE.md:21`（router 必须随 skill 增删改同步）、`.out-of-scope/question-limits.md` 与 `.out-of-scope/setup-skill-verify-mode.md`（不加冗余机制、两种失败来源的分诊）。

### 备注

- `SKILL.md` 末尾版本戳 v2.1.0 存在于文件但 CHANGELOG 缺该条目——既有记录缺口，非本轮引入；本轮改动记为 v2.2.0。

## [v2.0.0] - 2026-09-02

### Changed

- `guide-skill-auditor` 改为 `skill-auditor`，范围从 router 型 guide 扩展到 router、executor、reference、maintenance 四类 skill。
- 合并 memory 中已验证的 skill 原则：宿主事实解耦、模型无关、来源审计、插件优先、冷热证据、Pi 原生包优先、反向测试和运行时验证。
- 保留 router 十查、动态基线和组合审查；新增通用十查、executor 追加检查、skill 设计流程和统一输出契约。
- `test-prompts.json` 增加新建、来源、模型解耦、宿主解耦、组合和运行验证场景。

### Evidence

- 本地 memory：`skill-routing-boundary`、`custom-skill-source-boundary`、`skill-source-audit-use-ledger`、`model-agnostic-by-default`、`course-route-respect-completed-evidence`、`verify-actual-behavior-not-stated-config`、`verify-migration-config-end-to-end`、`check-pi-packages-before-designing`。
- 原 v1.x router 审计事故、动态基线和组合审查记录，见下方历史条目。

### Migration

- 活跃路由改指向 `skill-auditor`。
- `guide-skill-auditor` 旧目录已移除；旧 eval 和维护资产保留在新目录。
