...
> 本文件从 Claude Code 的 CLAUDE.md 迁移适配，规则同源。术语映射：Claude Code->Codex，CLAUDE.md->instructions.md，settings.json->config.toml。skill 由 cc-switch 同步到 `~/.codex/skills/`。

## 0. 基本约定

**语言：**以中文回复。

**试验场：**自建一个 `lab-area` 目录（如 `C:\<你>\Code\lab-area`）--专门试 skill/插件/hook/prompt，不污染其他项目。试验前先建子文件夹（`exp/<日期>-<主题>/`），产物放进去，完了一次性删，不污染根目录。正经子项目例外：自己就是个完整包，按包结构组织。

**AI 助手 learning-first 默认：**所有 AI 编程/研究助手调用默认 learning 优先（保持用户认知参与，按"这技能该不该归你"选分支：a)只问概念自己写 b)AI生成+你追问读懂 c)#TODO暂停你写一段；先后手不是关键，参与才是）。学习是目的，不是附赠。OFF 仅在用户显式 override（"做完就行""别解释""just ship""我不在乎过程"等），粒度 per-turn。**优先级：**实质学习点 -> learning-first 优先；纯交付/机械任务（1-2步单文件、bug修复、重命名、文案/客服/订单/上架/发货/小红书/抖音等对外产出）-> 默认 OFF，无需触发词。

**场景路由**：编码 -> `ai-coding-guide`；中文技术文章写改审 -> `article-writing-guide`；学习调研 -> `learning-guide`；UI/视觉 -> `frontend-guide`。多域任务逐域调路由；用户点名其他 skill 则尊重。（skill 由 cc-switch 同步到 `~/.codex/skills/`）
...
- 本机覆盖：个人本机偏好走 Codex 的 config.toml + 项目级配置（官方机制），不建臆造文件名。
- **GateGuard 拦非代码 Edit**：编辑 instructions.md/config.toml 等配置被 Fact-Forcing Gate 拦时，声明「无代码 import/无公共函数/无数据文件读写」+ 引用户指令原文，重试即过。
...
- 轮次 > 10 或连续 3 次 error -> 提示用户压缩上下文（AI 不能自触发）。
- 机械改动（重命名/单文件 bug/纯格式）-> 提示用户开新线程，不塞进已有长线程。
- Agent 犯新错误 -> 写入 memory feedback。
...
长任务、完整文件、多组件交付前激活 `full-output-enforcement` skill。禁止占位符（`TODO`/`// ...`）、散文敷衍词（"for brevity"/"rest follows same pattern"）、结构偷工（骨架当完整实现）。规则细节以 skill 为准。
...
- 3+ 步任务 / 多文件改动 / 用户给编号清单 -> 用 PROGRESS.md 或任务清单跟踪，逐步标完成。
- 1-2 步、单文件、纯问答 -> 不建。
- 开工即标 in_progress，做完即 completed，不攒批。

---

## caveman 句型

句型=[事物][动作][原因]。[下一步]。切级用对应命令，"normal mode" 关。
