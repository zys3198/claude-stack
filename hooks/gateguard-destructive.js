...
function editGateMsg(filePath) {
  ...
  return [
    ...
    '',
    '1. List ALL files that import/require this file (use ctx_search)',
...
function writeGateMsg(filePath) {
  ...
  return [
    ...
    '2. Confirm no existing file serves the same purpose (search the tree with ctx_tree/ctx_search)',
    '3. If this file reads/writes data files, show field names, structure, and date format (use redacted or synthetic values, not raw production data)',
    "4. Quote the user's current instruction verbatim",
    '',
    'Present the facts, then retry the same operation.'
...
// Standalone entry (ecc 原先经 run-with-flags.js 包装，自建版直接跑本文件)。
// run() 放行时返回 rawInput(string)，deny 时返回 { stdout, stderr, exitCode }。
