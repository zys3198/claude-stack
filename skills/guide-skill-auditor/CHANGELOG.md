...
- **第 3 项盲区扩展**：第 3 项「不用于」清单扩展为「不用于清单 + description 路由目标列表」两处范畴词同查；修复分级 P1 行补第 3 项路由目标列表盲区（与「不用于」范畴词同性质、无实锤误路由）；test-prompts.json 新增 id 7（learning-guide 范畴词场景，验证新第 3 项能判 FAIL）。证据：2026-08-07 组合审查 learning-guide「路由到…等」、frontend-guide「各风格预设」踩范畴词，旧第 3 项只查「不用于」清单漏判。

### Fixed

- **自身 description 合规第 1 项**：删方法摘要「十项静态检查 + 子代理路由基线测试 + 按证据分级修复」+ 补 what 句「本 skill 是 guide skill 质量审查器」。证据：auditor 用自己十查自审发现自身 description 违反第 1 项（含流程摘要 + 缺 what），14 天未被发现。
- **MAINTENANCE.md 术语残留**：L13/14「九查」改「十查」（v1.2.1 声称已改实测未净，假绿灯）；L44/47 变更记录历史叙述保留原样（历史就是历史）。

### Rationale

- 用户「先优化 auditor 再优化四个 guide」驱动，先修审查工具自身硬伤再审四个 guide。Q1=B（硬伤 + 补盲区）、Q3=A（不加体量检查）。
- 第 3 项扩展满足十查门禁（增删检查项须引真实事故）：learning「等」+ frontend「各风格预设」两起实测事故。
- 不加体量检查项：用户 2026-07-31 说 ai-coding-guide 先不瘦身，体量硬指标难定，加了诱导盲目删内容。

### 未动区域（防无证据漂移）

- 十查其他 9 项、动态基线流程、输出格式、落账规则、反模式表、组合审查五维：均未改。第 3 项是扩展非替换，原有「不用于清单范畴词」检查保留。
