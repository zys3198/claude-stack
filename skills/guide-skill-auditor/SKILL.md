...
| 7 | **默认值先于菜单** | A/B/C 仅在「路径选择改变成本/风险/产物且无法判断」时出现；用户意图明确时直接给默认路径 | Anthropic defaults-not-menus |
...
用户问「skill 的组合/搭配是否合理」时，除逐 guide 跑十查外，加审五维（主会话即可，子代理故障降级同第 2 步）：

1. **域边界覆盖**：四域拼起来有没有两不管地带（翻译/录屏/运维等第五域信号归谁）。
2. **边界重叠**：正向触发词重叠时 tie-breaker 是否四家一致（看交付物/目标）。
3. **跨界交接一致性**：A 说转 B、B 是否对称接 A；无死循环、无转介进虚空。
