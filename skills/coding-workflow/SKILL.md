...
- 影响分级由主模型直接按 R0-R4 完成：R0/R1/R2 的明确、私有、可逆操作直接执行；R3/R4 的共享资源、生产、真实数据、远程 Git、外部发布、删除、迁移和真实用户可见变更进入确认线。确认后的授权登记必须绑定 `session_id`、`task_id`、精确 `targets`、`operation_family`、`impact_ceiling` 与 `critical_params`；切换 task 清空旧授权，状态异常按未授权处理。
...
- 非平凡任务先读最小上下文，目标/约束/验收缺失先问；机械改动不反问。需求或架构未定时按 `mattpocock-skills:grill-with-docs`（有仓库）或 `grill-me`（无工作目录）澄清，并记录术语、决定和依据。区分 Prompt、Context、Harness，不让模型猜缺口。
...
沿用宿主实际契约；没有契约时计划至少列任务、范围、验证和回退。需要规格时手动调用 `mattpocock-skills:to-spec`；规划产物完成后标记完成，后续不把旧计划当现行指令。每条验收项必须一眼可判定。

## 1.5.1 深模块与接口归属

设计/重构模块形态时走 `mattpocock-skills:improve-codebase-architecture`，并读 `mattpocock-skills:codebase-design`；接口归人、实现归 AI、测试负责诚实；一个模块一个目录，对外只暴露接口文件。
