...
| 2026-08-06 | v1.4.5：清 `planning-and-task-breakdown` 残留死引用 3 处（ecosystems.md 2 处 + test-prompts id:3）；ecosystems.md Matt 段重写补双形态并存（cc-switch 裸名 vs 插件版 `mattpocock-skills:` 24 个）+ 主流程骨架 + user/model-invoked 差异，删过时自述；版本戳对齐 v1.4.5 | guide-skill-auditor 审查：十查 #6 详查主文件零幻觉（`grill-me` 等 cc-switch 裸名 model-invoked 可调），但 ecosystems/test-prompts 残留 v1.4.3 已删 skill 死引用（P0）；Matt 生态地图漏插件版（P1）；版本戳脱节（P2）；动态基线 5 场景零误路由。详见 CHANGELOG [v1.4.5] |
| 2026-08-06 | v1.4.6：「开发新功能」+「有需求文档」两分类加 Matt 流程转介提示（命中 Matt 独有信号→提示手动 `/mattpocock-skills:ask-matt`，只提示不抢路由）；版本戳 v1.4.6 | 用户拍板「ask-matt 重点用，B 主 A 辅」：guide 转介接线 + 用户手动主力；ask-matt 两形态均 user-invoked 模型调不到，只能转介；方案 1 最小改不新增分类防膨胀；audit P0:0 P1:0 P2:2（均假阳性）。详见 CHANGELOG [v1.4.6] |
