...
2. **再看顶层独立 skill**：`~/.pi/agent/skills/`（自建）+ `~/.pi/agent/skills-sync/`（sync 第三方）。
...
6. **并行执行层（可叠加）**：主分类确定后，若存在两个以上互不依赖且可单独验收的子任务，叠加 `parallel-delegation`；它只负责 handoff、worker 隔离和主代理验收，不替代分类、Matt 主流程、交付状态机或 review。单文件机械、强耦合、共享写入或用户明确要求单代理时不叠加。
7. **收尾固定**：产生产品代码改动后才进横切收尾；纯理解、纯审查、纯文档不强行 verify。
...
| 任务跨会话、多人、需要并行或需要显式阻塞关系 | `/to-tickets` | 提醒用户手动调用并等待；按用户可验证结果拆 Ticket；同会话内仅需独立执行时，可在主路径确定后叠加 `parallel-delegation`；单会话无阻塞需求可跳过 |
