...
本 Skill 的两种模式都先定范围，只交草稿、报告或最小 diff，不直接修改项目文件。
...
用户拿方法论文章（AI 工作流、工程实践等）问「能为我做什么」时，先把每条主张归入三类，再谈落盘：

| 类 | 判据 | 去向 |
|---|---|---|
| AI 能自觉做 | 可写成规则或 skill 让模型自动执行的动作（如委派探索 subagent、TDD、缺信息停下问） | 才可能进 `CLAUDE.md` / skill 正文 |
| 只能人做 | 与模型能力无关、交不出去的判断和操作（业务取舍拍板、划定 out of scope、设计模块接口、视觉与手感 QA、沙箱与权限决策） | 只供人自己学习和保持警觉，**写进规则无效** |
| 人机协作 | 两侧各有固定动作且顺序不可换（grilling、spec、Kanban、代码审阅、QA） | 进工作流设计，用表格列清 AI 做什么、人做什么 |
...
合并、搬运、精简正文时**禁令句必须保持祈使语气**。把 `# 不要用 Browser(url)` 改写成 `# 整轮复用同一标签页`，看起来信息量没少，实际是把约束降级成背景说明——读禁令会改变行为，读描述只增加上下文，模型按签名自然度选 API。禁令要给出**违规的具体写法**，不能只说正确写法（违规那个往往签名更自然、更容易被写出来）；约束最好定位到源码行，冲突时可复核。
