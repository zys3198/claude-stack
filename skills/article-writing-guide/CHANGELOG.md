...
- 开工问询「模式」三选（coach/pair/driver）替换为「归属问（你练/我讲/我动手）」，说话层 persona 定义集中到 `learning-personas`，只引用不展开（渐进式披露）。
- §3 默认协作流「默认参与度 pair」→「默认归属=我讲（协作流）」；§4.6 同步。
- REFERENCE.md 同步换词。
- description 增补「协作参与度/说话层统一走 learning-personas」。

## [v1.7.0] - 2026-08-14

### Added（article-writer §6 降 AI 检测改写守则）

- **`article-writer/SKILL.md` 新增 §6.11「JavaGuide 模式降 AI 检测：改写守则」+ §6.12「降检测改写自检 checklist」**：R1-R11 十一条改写守则（元叙述删 / 设问改陈述 / 类比克制 / 句中加粗收敛 / 强词收敛 / 名词短语标题 ≥70% / 命令裸写 / 结构自述删 / 新增章节去模板化 / 转折二分壳收敛 / 篇幅不均）+ 12 项 grep checklist；§6.9 反例表追加行 12-16。
- **依据**：2026-08-14 朱雀 AI 生成检测对照实测——同一素材原文 ≈0.33（人工）vs JavaGuide 模式改写 ≈0.95（AI，8 片段 6 个 ≥0.95）；根因是「AI 教程腔密度」而非结构，保骨架删装饰。
- **冲突处理**：§6.11 开头声明降检测场景本节优先于 §6.1（导航字数提示）/ §6.3（Q&A 5-8 题）/ §6.6（过程类问句标题）；原条款未改，创作模式不受影响。
- **实测回填**：改写 `article-2.md` 应用 R1-R11 后，§6.12 checklist 12 项全 PASS；过程中修复 R6 grep 模式「`.*别` 误伤『识别/区别』」→ 改 `^###\s*(记住|别|先|务必|一定要)`。

### Rationale

- 依据：三 agent 学习结论（原文 / 改写 / 检测报告对照）+ 用户「确认」后落盘。
- 未动：§6.1/§6.2/§6.6 原文（冲突由 §6.11 声明本节优先，避免改动创作模式行为）。

## [v1.7.1] - 2026-08-14

### Changed（§6.11 复测修正 + R12-R16 骨架组件守则）

- **§6.11 根因修正**：原「检测器罚的是 AI 教程腔密度，不是结构」修正为「还罚结构脚手架（Q&A 章 / 导航 / 编号结尾 / 代码围栏 / ### 层级）」；新增「两种 JavaGuide 文体」框架——公众号原始文体（平铺体）低检测 vs docs/ai 教程文体高检测，降检测场景切换到平铺体。
- **新增 R12-R16**：Q&A 章整章删除 / 导航列表删除 / 结尾禁用编号清单 / 代码命令裸写去围栏 / 警示标签删除（R16 覆盖 R5 的「≤1 保留」）。
- **§6.12 checklist 追加 13-17 项**（Q&A 章 0 / 导航 0 / 编号结尾 0 / 围栏 0 / 警示标签 0）。
- **实测**：R1-R11 后复测 0.95 → 0.83（平铺段片段 4 @ 0.17 验证有效）；按 R12-R16 平铺改写后待复测。

### Rationale

- 依据：2026-08-14 朱雀复测（朱雀AI检测助手（2）.pdf）：10 片段加权 ≈0.83；Q&A+编号结尾区 0.997、导航+钩子区 0.93、裸命令区 0.50、平铺散文区 0.17——对照参考原文 74% 平铺正文 0.15。
- 决策：用户拍板「全面趋平铺」；并要求检测证据不写入 skill 正文（规则保留，证据留 CHANGELOG 备查）。

## [v1.7.2] - 2026-08-14

### Changed（复测收尾：文字版天花板确认）

- **第三次复测（朱雀AI检测助手(3).pdf，19:15）**：平铺版加权 ≈0.81（4 片段：0.9920 / 0.7620 / 0.4512 / 0.9518）。0.95 → 0.83 → 0.81，碎片化 10 → 4（平铺化有效），整体停滞 ~0.8。
- **结论**：纯文字改写版（无截图、无实跑亲历）检测天花板 ≈0.8；参考原文 0.33 主要靠满篇真实截图 + 作者实跑亲历构成，非文风可复刻。R1-R16 仍有效（降 AI 教程腔、减少碎片化），但单靠文风规则到不了 <0.5。
- **决策**：用户拍板收尾，接受 0.81 为定稿；不补图不补实跑。补图路径（真截图 + 实跑段落替换「截图和实跑体感我没有，不装」）留作可选后续。

### Rationale

- 依据：三轮复测证据链（0.95 → 0.83 → 0.81）+ 参考原文对照（0.33，截图/实跑密集）。证据全部留在 CHANGELOG，skill 正文不含检测数字（用户要求）。
