...
- **问询顺序**：
  1. **归属**：这技能归你吗？（你练 / 我讲 / 我动手，定义见 `learning-personas`）——想练写作核心（结构/论点/文体判断）→ **你练**；协作流（AI 给结构/方向选项你定，写作域常见默认）→ **我讲**；一次性文案/样板 → **我动手**（收尾讲为什么）；卡壳 -> AI 递参考判断
  2. **背景**（模糊时）：为什么写/给谁看/什么目的/有无参考；能查的不问
  3. -> 进 §1 路由表
- **例外（不问，直接我动手）**：纯排版/格式规范、已指名标杆、用户说"直接写"。
- **分工**：开工问询管归属+persona+背景；§4 决策点先问管文体方向；写作纪律管依据。
...
用户说"我想和你讨论结构后再写""你给我选项，我也会补充想法""逐步设计每节再生成""生成后我来审查给建议"这类需求时，默认走协作流，不直接一次性全文起草（**默认归属=我讲（协作流）**：默认就走，不需要用户主动说；除非开工问询定了我动手或你练，用户说"直接写"才降级）：
