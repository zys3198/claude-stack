...
│   ├─ 是 → article-writing-guide 协作流：结构讨论 → 结构冻结 → 逐节设计 → 对话初稿预览 → 用户确认 → 落盘/下游改稿 → 用户审查循环；执行端用 article-writer 协作模式
│   └─ 否 → 继续判断学习/素材状态
...
## 2. 默认协作写作流（结构 → 小节设计 → 生成 → 审查）

适用触发：用户说“先和你讨论结构”“你给我几个结构选项”“我想到什么再补充”“逐步设计每节内容”“设计完再生成”“生成后我来审查给建议”。

```
结构讨论 → 结构冻结 → 逐节设计 → 对话初稿预览 → 用户确认 → 落盘/下游改稿 → 用户审查循环
```

执行要点：

1. 先给 2-4 个结构方案，每个方案只说明适用场景和取舍，不直接写全文。
2. 用户可选择、混搭、增删章节；讨论到可执行后再输出最终大纲 + 每节目标。
3. 逐节设计时写清：本节要解决的问题、关键论点、证据/例子、读者获得感。
4. 每节设计完停一下，让用户补充或调整；不要默认一口气设计完整篇。
5. 小节设计确认后，先在对话里生成正文初稿并标注“初稿/待确认”；不要直接写入文件。
...
- ❌ 用户明确要先讨论结构，却直接用 `article-writer` 一次性全文起草。
- ❌ 大纲未确认就进入正文生成。
