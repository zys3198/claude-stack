# Article Writing Guide — 详细路由规则

SKILL.md 给一句话路由表；本文件覆盖**边界 case、决策树、多 skill 串联顺序、反模式**。agent 在 SKILL.md §4 规则4（模糊无法匹配）时读本文件。

成本标注（与 SKILL.md 一致）：🪶轻 / 🪨重 / 🔥烧 token。

---

## 1. 起草阶段决策树

```
用户要写文章
├─ 用户想“先讨论结构/你给选项/我补充想法/逐节设计/生成后审查”？
│   ├─ 是 → article-writing-guide 协作流：结构方案 → 方向确认 → 一次性展开全篇细骨架 → 讨论并明确冻结 → 逐节正文预览 → 用户确认 → 落盘/下游改稿 → 用户审查循环；执行端用 article-writer 协作模式
│   └─ 否 → 从零创作或深度改写仍先走全篇细骨架闸门；仅用户明确要求“直接写/跳过骨架”时绕过，再继续判断学习/素材状态
├─ 用户想”边学习边写”或”从 0 了解项目/工具后成文”？
│   ├─ 是 + 有现稿 → agent-reach/lean-ctx 抓权威资料 → 轻量学习路径/心智模型 → 对话中给增量初稿 → 用户确认后 edit-article 写入
│   ├─ 是 + 无现稿 → agent-reach/lean-ctx 抓权威资料 → 学习记录 → 对话初稿预览 → 用户确认后 article-writer 起草/落盘
│   └─ 否 → 继续判断素材状态
├─ 有现成素材（笔记/碎片/旧稿）？
│   ├─ 是（碎片/笔记/粗稿）→ article-writer 协作模式（§7：开工问询 → 结构方案 → 全篇细骨架 → 明确冻结 → 逐节生成）
│   └─ 否（从零）→ 看文体
├─ 文体？
│   ├─ 博客/方案/知识库/面试题/工具推荐/面经/单篇教程 → article-writer 🪨
│   ├─ 对外发布的系列教程/从零系统学习/课程计划 → tutorial-maker 🪨（给自己学会 → 转介 learning-guide）
│   ├─ 强叙事、想边写边选支线 → article-writer 协作模式 🪨（叙事节拍由 §7 协作模式承接）
│   └─ JavaGuide 项目内（docs/ai 系列、my-submission/） → article-writer 🪨（JavaGuide 模式，执行规范见其 §6，终检判定见 javaguide-style-guide）
└─ 需要先调研？
    ├─ 本仓库 .md → lean-ctx 🪶
    ├─ 全网某话题近 30 天热度/讨论 → last30days 🪶（Reddit/X/HN/YouTube/TikTok）
    ├─ AI 行业资讯/日报/热点 → agent-reach 🪶（公众号/多平台，中文）
    ├─ 多平台定向检索（小红书/抖音/微博/B站/公众号…） → agent-reach 🪶
    ├─ 全网多源带引用 → deep-research 🔥（系统内置，本机未装；先 SKILL.md §4 规则7 校验存在）
    └─ 产品/公司/人物深度研究 → agent-reach 🪶 / ecc:deep-research（插件条件路径）
```

## 2. 默认协作写作流（结构方案 → 全篇细骨架 → 冻结 → 逐节正文 → 审查）

适用触发：写作域默认我讲（协作流）；用户说“先和你讨论结构”“你给我几个结构选项”“我想到什么再补充”“设计完再生成”“生成后我来审查给建议”时尤其明确。用户未主动说这些话，只要是从零创作或深度改写，也默认经过全篇细骨架闸门；明确要求“直接写/跳过骨架”才绕过。

```
结构方案 → 方向确认 → 一次性展开全篇细骨架 → 讨论修改 → 明确冻结 → 逐节正文预览 → 用户确认 → 落盘/下游改稿 → 用户审查循环
```

执行要点：

1. 先给 2-4 个高层结构方案，每个说明适用场景、主线和取舍，不直接写全文。
2. 用户选择、混搭或增删章节后，复述文章目的、读者、核心观点、事实边界和目标篇幅，只确认展开方向。
3. 按选定方向一次性展示完整 H2/H3；不得把粗目录或“最终大纲 + 每节目标”当成冻结完成。
4. 每个 H2/H3 写清：本节问题、关键论点、依据 ID、证据/例子（缺失标 `needs evidence`）、读者获得感、目标篇幅、表现形式、前后过渡；深度改写再加现稿位置映射。
5. 骨架末尾汇总预计总篇幅、证据缺口、待补真实经历/截图和可合并章节，便于从全篇视角排查重复、依赖和篇幅失衡。
6. 用户讨论修改后必须明确表示“骨架确认/按这个写/冻结”，才进入正文；方向确认或只确认某节不算冻结。
7. 按冻结骨架逐节生成正文，先在对话里标注“初稿/待确认”，同时交付依据表和 basis map；结构要变则回到骨架阶段重新冻结。
8. 用户确认正文后才落盘或调用会改文件的下游 skill；长文按节落盘，不把未确认章节写死。
9. 用户审查后先处理结构问题，再处理论点/例子，最后处理文风和排版。

与下游 skill 分工：这是 `article-writer` 前置协作层；需要调研先跑 `lean-ctx`/`agent-reach`/`deep-research`，需要去 AI 味再跑 `human-writing`，发布前手动终检。

### 不要做

- ❌ 从零创作或深度改写时，在全篇细骨架冻结前生成正文；用户明确要求跳过除外。
- ❌ 只给粗目录或每节目标，就把它称为“结构冻结”。
- ❌ 恢复成“每设计一节就停一下”，导致用户始终看不到全篇依赖与重复。
- ❌ 全篇细骨架未一次性展示、未明确冻结，就进入正文生成。
- ❌ 正文初稿未给用户看，就直接写入文件或调用会改文件的下游 skill。
- ❌ 用户给审查意见后直接无边界重写全文。
- ❌ 把协作流当成替代调研/查重/发布闸的万能流程。

## 3. 边学边写路径（学习记录自然成文）

适用触发：用户说“边学习边写”“从 0 了解 X”“以官网/官方文档为准学一个开源项目”“像刚才那样把学习记录写进文章”。

### 路线 A：已有现稿

```
权威资料收集 → 学习路径/心智模型 → 按现稿标题拆结构 → 对话中给增量初稿 → 用户确认 → 写入对应章节 → 事实审校
```

执行要点：

1. 先用 `lean-ctx`（本地资料）或 `agent-reach`（官网/GitHub/网页）抓权威来源。
2. 先产出学习路径，不急着改文。
3. 读现稿标题树，按现有结构找插入点。
4. 先在对话中给增量初稿，重点补心智模型、命令路径、踩坑点、阶段验收，不大幅推翻原结构。
5. 初稿展示前先做轻量事实自查；用户确认后再写入对应章节。
6. 写入后手动做正式事实审校（逐段核对官方来源），避免学习过程中的临时理解沉淀成错误结论。

### 路线 B：没有现稿

```
权威资料收集 → 一天/多天学习路线 → 学习记录沉淀 → 对话初稿预览 → 用户确认 → article-writer 起草/落盘 → edit-article 调结构
```

执行要点：

1. 学习记录先按“问题 → 资料证据 → 我的理解 → 可复用表达”组织。
2. 先在对话中给初稿预览，保留学习现场感，避免写成百科词条。
3. 用户确认后再让 `article-writer` 起草/落盘。
4. 最终文章结构按读者路径走：先会用，再懂设计，再能落地。

### 不要做

- ❌ 一边学一边把所有资料原样堆进文章。
- ❌ 学习路径还没形成，就直接让 `article-writer` 写完整稿。
- ❌ 有现稿时推翻结构重写，除非用户明确要求重构。
- ❌ 没给用户看增量初稿，就直接把学习记录写入文件。
- ❌ 把临时猜测当事实写入；无官方来源就标不确定或留待核查。

## 4. 改写 vs 润色 vs 规范化（最易混）

| 用户原话 | 真实意图 | skill |
|---------|---------|-------|
| "结构乱""重新组织""逻辑不顺""章节调整" | 改**骨架** | `edit-article` 🪨 |
| "AI 味重""不像人写的""文风生硬""改自然点" | 改**肉**（措辞/语气） | `human-writing` 🪨 |
| "中英文空格""标点不对""标题层级乱""引号格式""批量规范" | 改**皮**（排版） | 手动规范（统一中英文空格/标点/标题层级） |
| "深度改写整篇""重写这篇""骨架和措辞都要动" | 改**骨架+肉**（整体重写） | `article-writer` 🪨（深度改写模式） |
| "帮我改改这篇"（模糊） | 不确定 | 先问改哪层；或按 骨架→肉→皮 顺序串行；若要整体重写走 `article-writer` |

**串行顺序**（两层都要改时）：`edit-article` → `human-writing`。先动骨架会牵动措辞；排版最后手动规范避免反复改格式。

## 5. 审校与发布闸（skill 已退役，手动执行）

审校类 skill（`tech-article-review` / `multi-review-pipeline`）与发布闸（`publish-final-check`）已随 2026-08 审计退役，当前**手动执行**：

- 用户说"逐段 review / 快速过一遍" → 手动逐段审校，标 VERSION。
- 用户说"全面审稿 / 审完顺手改 / 先出报告" → 手动通读 + 给修订清单，按用户确认再批量改（可停在报告阶段）。
- 用户说"发布前过一遍 / 能不能发了" → 手动终检（死链/查重/风格硬伤）；JavaGuide 模式用 `javaguide-style-guide` 判定风格。
- （`review-doc` 已按 2026-08-13 审计+用户拍板移出路由表。）

## 6. 配图两件套顺序

铁律：**先决策后画图**。

1. `drawio-article-illustration` 🪶 — 读正文，判断：哪里**真正需要**图（避免装饰图）、用什么图型（流程/架构/时序/状态机/ER）。输出"配图清单 + 类型建议"。
2. `drawio-chart` 🪨 — 按清单实际画，导出 PNG/SVG/PDF。
3. 画完回到 `drawio-article-illustration` 做**图文一致性校验**（图与正文是否对得上）。
4. **图注一致性**：每张图补图注/编号（图1、图2…），正文引用处与图注编号对齐；中文文章 alt 文本用中文简述，不空、不写文件名。

禁止：跳过 ① 直接 `drawio-chart` 画装饰图。

## 7. 查重前置条件（只比贴源模式）

**手动查重**（原 `plagiarism-audit` 已退役）**只比用户贴的参考源**，不主动搜全网。前置：

- **必须**用户提供外部来源（URL / 文件路径 / 粘贴文本）。无来源 → 先问用户对齐哪些来源，或先用 `lean-ctx`/`agent-reach` 拉候选来源。
- 用户明确声明"无参考源" → 跳过查重，但终检报告标 `⚠️ 未查重`。

**判定边界**（两类文本区别对待）：
- **技术事实描述**（如"TCP 三次握手步骤""HTTP 状态码 200 含义"）→ **允许相似**，不必改。事实就是事实。
- **原创措辞搬运**（比喻/结构/举例/表达方式直接搬）→ **必改**。即使观点参考，措辞必须原创。

**JavaGuide 铁律**："只可参考不可抄袭"——参考要点 OK，但措辞、结构、举例必须原创。

**改写示例**：

| 原文（参考源） | 抄袭版（必改） | 原创版（OK） |
|---------------|---------------|-------------|
| "RAG 就像是给大模型开卷考试" | "RAG 如同给大模型开卷考"（同比喻换词） | "RAG 的本质是把检索当外挂记忆，模型答题前先翻资料"（换比喻+加判断） |
| "Agent 记忆分短期和长期" | "Agent 的记忆有短期和长期两类"（换虚词） | "Agent 记忆按生命周期切：当前会话的临时缓存 vs 跨会话的持久化沉淀"（换框架） |

**关键**：换同义词不算原创，换比喻/结构/视角才算。

## 8. 完整 pipeline 串联（含发布闸 + 确认后落盘规则）

通用技术文章发布前，按序执行；涉及起草、改写、批量改的步骤先对话预览，用户确认后才写文件：

1. `article-writer` 🪨 起草（系统教程/系列课用 `tutorial-maker`；素材成型走 `article-writer` 协作模式）
2. `edit-article` 🪨 调结构
3. `human-writing` 🪨 去 AI 味
4. `drawio-article-illustration` 🪶 + `drawio-chart` 🪨 配图
5. 手动审校 + 批量改
6. 手动查重（只比贴源，**需外部来源，否则跳过**）
7. 手动排版规范
8. **手动终检**（发布前人工过一遍：死链/查重/风格）

JavaGuide 模式：步骤1 切 JavaGuide 模式（执行规范见 `article-writer` §6）；步骤6 对齐"只可参考不可抄袭"；步骤8 风格判定对照 `javaguide-style-guide` §1 M1-M10 + §2 量化阈值。

> **路径稳定性**：`my-submission/`、`docs/ai/` 等 JavaGuide 关键词以**目标仓库实际目录为准**（仓库重构时可能改名/迁移）；命中 JavaGuide 模式前先确认目录存在，别凭关键词硬套。

**确认后落盘规则**（断点续跑 / 版本回滚用）：

默认先在对话中展示初稿/改稿预览。只有用户明确说“写入”“落盘”“应用到文件”“按这个改”后，才创建或修改文件。

```
<slug>/
├─ draft.md                 # 步骤1 起草产物
├─ draft-v2-structured.md   # 步骤2 调结构后
├─ draft-v3-polished.md     # 步骤3 去AI味后
├─ draft-v4-illustrated.md  # 步骤4 配图后
├─ draft-v5-reviewed.md     # 步骤5 审校改后
├─ draft-v6-final.md        # 步骤6-7 查重+排版后
└─ publish-report.md        # 步骤8 手动终检报告
```

规则：每步输出先给对话预览；用户确认后再写独立文件，不覆盖上版；回滚直接取上一版；最终发布物 = `draft-v6-final.md`（手动终检通过后）。

## 9. 反模式（不要这样做）

- ❌ 用户说"写篇文章"就直接徒手写 → 应委派 `article-writer`。
- ❌ 涉及起草/改写/边学边写时，未给用户看初稿就直接写文件 → 应先对话预览，确认后落盘。
- ❌ 从零起草或深度改写只列粗目录就开写 → 应先一次性展开全篇 H2/H3 细骨架，讨论并明确冻结。
- ❌ 用户说"去 AI 味"用 `edit-article` → 应 `human-writing`（edit-article 改结构不改文风）。
- ❌ 跳过 `drawio-article-illustration` 直接画图 → 易产装饰图。
- ❌ 无外部来源就手动查重 → 会空转（用户声明无源才跳过）。
- ❌ 把本路由器当执行器（自己重写各 skill 能力） → 本 skill 只**定路由**，执行委派给具体 skill。
- ❌ 审校时把查重当审校 → 查重只比贴源，审校独立逐段过。
- ❌ 跳过发布前终检直接发布 → 终检缺失，死链/查重/风格硬伤可能漏网。
- ❌ 查重主动搜全网 → 手动查重**只比贴源**，主动搜未启用。
- ❌ 混淆 `javaguide-style-guide` 与 `article-writer` §6 → 前者**终检判定**（M1-M10 + 阈值），后者**写作执行**（含 §6.7 自检、§6.9 反例）。

## 10. 路由失败兜底

读完本文件 + SKILL.md §7 端到端示例仍无法定路由 → 反问用户三要素：
1. **文体**：博客 / 教程 / 方案 / 知识库 / 面试题 / 面经 / 叙事？
2. **阶段**：从零写 / 改现稿 / 审校 / 查重 / 配图 / 发布闸？
3. **目标仓库**：通用 还是 JavaGuide（docs/ai 系列）？

拿到答复后回 SKILL.md §1 表定位。

> 端到端路由示例见 SKILL.md §7（query → skill → 理由 10 条）。测试用 prompt 见同目录 `test-prompts.json`。
