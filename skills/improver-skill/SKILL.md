...
- 运行时可观测时，分别记录固定注入、路由描述、正文读取和回流材料的上下文成本；无法观测写 `not-run`，不把估算值填为通过。上下文成本用于解释收益和回归，不替代 `target`、`guardrail`、`holdout`。
...
证据分三层记录：静态内容与 hash、运行输出与命令、任务结果与使用信号。文件存在、配置声明、成功加载或命令返回成功都不能替代结果证据。未知状态不填成 `passed`。
