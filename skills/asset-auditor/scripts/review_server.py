#!/usr/bin/env python3
"""Loopback-only review runtime for the asset-auditor workflow."""
...
def resolve_state_root(value: str | None) -> Path:
    ...
    host_root = Path(configured).expanduser() if configured else Path(__file__).resolve().parents[3]
    return host_root / "skill-trimmer"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve_parser.add_argument("--state-root", help="状态目录固定在安装根目录下的 skill-trimmer/（内部标识，不随 skill 改名）。")
    ...
    for name, help_text, func in (
        ...
        child.add_argument("--state-root", help="状态目录固定在安装根目录下的 skill-trimmer/（内部标识，不随 skill 改名）。")
        if name == "read":
