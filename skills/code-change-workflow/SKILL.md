...
- **非平凡任务先过 Context→追问→执行环**：先读最小必要上下文；目标、约束或验收缺失时先问 1-3 个反向问题，补齐后再生成/修改；明确的机械改动不反问。典型循环：背景输入 → AI 反问 → 补齐上下文 → 产出初稿 → 人工复核 → 迭代。
...
- **护栏靠 hooks 不靠自觉**：危险命令拦截（rm -rf / DROP TABLE / 密钥外泄）由宿主实际挂载的 PreToolUse hooks 负责（以 `settings.json` 挂载为准），不靠“按场景授权”的人肉自觉。长链编排（>3 agent）使用项目或宿主已定义的执行计划/工作流合同协调并设步数上限；没有时记录最小计划，超限即停，防 agent 无限烧。
