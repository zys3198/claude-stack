...
本文件只在维护 `frontend-guide` 时使用。主文件负责运行时路由；本文件只负责维护规则、证据源和变更记录。
...
| 触发信号 | 必做动作 |
|---|---|
| 新装或卸载前端/设计类 skill / 插件 | 复核 `SKILL.md`「先定阶段 / 再定风格 / 重叠裁决」三表 + `test-prompts.json` 样例 |
...
| 路由决策改了阶段主路径、风格预设或重叠裁决 | 同步 `SKILL.md`、`test-prompts.json` |
| 兄弟路由器（ai-coding-guide / article-writing-guide / learning-guide）边界改动 | 复核 description「不用于」清单与「与兄弟路由器」段 |
...
1. 扫当前会话可用项和本地安装项
2. 先修 P0：死 skill / 错默认路径 / 错归属（如把设计 skill 归给编码域）
...
- 与编码域抢单类事故优先：「页面/界面/UI/落地页/登录页」信号被 ai-coding-guide 抢走是本指南最高风险失败模式（2026-07-22 RED 基线实锤）
...
| 2026-07-31 | v1.5.3：删 description「按阶段与风格」流程词；删风格表 `stitch-design-taste` + 重叠裁决 `design-taste-frontend-v1`（已移入 _weak-model-backup） | guide-skill-auditor 组合审查 #1 borderline + #6 状态矛盾；详见 CHANGELOG [v1.5.3] |
