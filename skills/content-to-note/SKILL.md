...
description: 把公众号/B站/抖音链接提取整理成结构化 Markdown 笔记,自动识别来源路由。用户分享 mp.weixin.qq.com / bilibili.com / b23.tv / v.douyin.com / douyin.com 链接,或说"提取/整理/做笔记/存一下这篇/记成笔记"时使用。统一落盘到 wiki(公众号进 71-wechat-articles、视频进 70-video-notes),frontmatter 统一。各来源底层提取由专用 skill 或脚本负责,本 skill 只做路由 + 协调 + 统一笔记规范。触发:提取公众号、整理B站视频、这个链接做笔记、存一下这篇、帮我记成笔记。
---
...
| 来源 | URL 模式 | 提取引擎 | 笔记目录 |
|------|---------|---------|---------|
| 微信公众号 | mp.weixin.qq.com | wechat 脚本(见下) | wiki/71-wechat-articles/ |
| B站视频/图文 | bilibili.com, b23.tv, BV… | bili-note skill | wiki/70-video-notes/bilibili/ |
| 抖音 | v.douyin.com, douyin.com/video | douyin-video-summary skill | wiki/70-video-notes/douyin/ |
...
```powershell
node C:\Users\zys31\.agents\skills\content-to-note\scripts\wechat\run.js "<mp-url>" "<wiki/71-wechat-articles/YYYY-MM-DD-标题.md>"
```
...
用法/原理另见:wiki/71-wechat-articles/README.md。

### B站

调 bili-note skill(`~/.codex/skills/bili-note`)。它有完整流程:元数据 / 字幕 / ASR / 评论 / 归档 / 预算 / 评分。

边界:bili-note 的**原始材料归档**(默认 D:\knowledge\…)保持原处不动,本 skill 只把整理后的**学习型笔记**存进 wiki/70-video-notes/。

### 抖音

调 douyin-video-summary skill(`~/.agents/skills/douyin-video-summary`)。流程:浏览器取音频 → whisper 转写 → 摘要。
