...
- 保鲜（想审计时跑）：在宿主的 `skills/` 目录中执行 `python skill-trimmer-by-user/scripts/scan_skills.py` → 扫自建 skill → `inventory.json` 每 skill 带 `usage_count`/`last_modified`/`staleCandidate`，stale 候选直接列在 stdout；去留拍板仍走本 skill 判定流程。
