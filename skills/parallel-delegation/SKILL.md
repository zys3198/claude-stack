...
1. 先判断子任务是否独立、边界是否清楚、结果是否可单独验收，以及收益是否超过成本。收益有两笔：并行省下的时间，以及主代理窗口省下的上下文——探索消耗留在 worker 自己的窗口里，只把摘要带回来；成本是钱照付，加上协调与验收开销。需要跟主代理已有上下文一起权衡的决策不委派：worker 看不到主代理的窗口，做出来的取舍缺前提。
...
4. 涉及模型、effort、并发、隔离能力或失败处理时读取 [`references/runtime-and-failure.md`](references/runtime-and-failure.md)，并按下面的 Configuration gate 取得确认。
5. 整合前读取 [`references/verification.md`](references/verification.md)，按风险完成主代理复核和最终报告。
...
准备选择或调整子代理配置，或需要并发、共享资源、外部访问时，先向用户明确展示：拟选模型（可见时写实际名称）、provider、route、effort、并发数和隔离方式；判断配置是否合理，说明原因、影响和备选方案；取得确认后再启动。常规单个委派沿用当前会话默认配置直接执行，不重复询问。宿主未暴露的值写 `unverified`，已确认同一配置时不重复询问。
