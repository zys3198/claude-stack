...
两条目的：**限制占用**（单个容器不能吃光本机内存与进程数）；**安全性**（容器内进程不得提权，宿主机的独占资源同一时刻只有一个使用者）。
...
- 一次性容器用 `--rm`；常驻服务用 `docker compose up -d` 并登记台账。
...
`docker exec` 进去产出的东西同规矩：只有落在挂载点之下才算保住。
...
每次任务结束前盘点本次创建或修改的容器、网络、卷、镜像、构建缓存、日志和挂载目录产物；一次性容器用 `--rm` 自动清理；常驻服务可直接执行 `docker compose down --remove-orphans`。
...
**分界。** 会变的（去处选择、资源上限取值、授权边界、清理流程、「机械保证」所述 hook 行为）整体重写。只增的：本文件没有——依据留在 `resource-guard.py` 的头部注释里，正文不留历史。
