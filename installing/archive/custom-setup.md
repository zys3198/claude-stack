...
### 正文二次优化：确凿错误 + 维护条款去重（2026-09-25）

- **变更**：7 文件 14+/28−，净 −1,736 B。`article-writer/SKILL.md` 9,813 → 9,632（−181，删「caveman 输出纪律」）；`docker-only/SKILL.md` 5,947 → 5,613（−334）；`parallel-delegation/SKILL.md` 4,821 → 4,507（−314）；`task-notes/SKILL.md` 8,139 → 7,728（−411）；`docs/protocols/gate.md` 4,505 → 4,169（−336）；`docs/protocols/memory.md` 4,036 → 3,717（−319）；`docs/protocols-index.md` 4,676 → 4,835（+159，改共同要求第 6 条与「维护条款」导言，使其成为删除判据与触发点的唯一来源）。
- **依据**：① `settings.json` 的 `enabledPlugins["caveman@caveman"] = false`，caveman 模式在当前配置下不存在，`article-writer` 那条指向的是停用插件。② 五份协议的「维护条款」里「二、删除判据」「三、触发点」逐字相同，`docs/protocols-index.md` 已载全份；按单一真相源收敛到索引，协议正文只留因文件而异的「分界」+ 一行指针。
