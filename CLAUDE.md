...
- **Edit 预读只认原生 Read**：ctx_read（MCP）已读不算，Edit 前必须原生 `Read`。
...
署名代码 = 你负全责，与 AI 写了多少无关。交付前逐项过，任一项不过 = 不交付。

- [ ] AI 写的每行能解释「为什么这么写」；解释不出的已追问到懂（learning-first，§0）
- [ ] 亲手验证过：编译/测试/页面实点，有输出或截图为证；不转发「应该没问题」
- [ ] `git diff --stat` 对得上任务清单：改动范围 = 任务范围，无顺手修改
- [ ] 不可逆操作（删数据/迁移/force push/密钥/生产脚本）逐条确认后才执行（§1 人工确认线）
- [ ] 确认性结论有证据（源码 > 官方文档 > 实测，§12）；无证据的已标「不确定」
- [ ] bug 修复查过所有调用方，修根因不修症状
- [ ] 能一句话说清本次改动的数据如何流动、流动过程是否符合正确业务逻辑（业务闭环）
