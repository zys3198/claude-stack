...
- 关键设计决策有压测/benchmark 数据支撑（无数据 → 不接受结论，让 AI 跑）
- 边界条件走代码路径核对一遍（空集/重复/类型不符/超阈值）
- 底层结构选型与「为何不选另一种」能讲清（dict vs map、intset vs 数组等）
- 审查报告输出走 §10 完整性约束

**沟通技巧（与 AI 对等对话）：**
- 抛见解 + 主动问 AI「你的看法」，避免单向指令造成方向偏差。
- 让 AI 用压测/可运行代码印证或证伪假设，不接受「应该/可能/大概」。
- 自己先读、先怀疑、再让 AI 印证——这是保住判断力的古法习惯，前期慢但复利。

**权限归属（按 §6）：** 单 reviewer = 询问确认；三 agent 对抗 = 必须确认。

**反模式：**
- 「读着顺 → 合入」：AI 最擅长的恰是「看起来对」。
- 「AI 自己 review 自己」：无对照基线，等于没审。
- 「靠 harness 自动验收代替人工闸门」：自动验收覆盖功能链路但替代不了设计意图判断。
