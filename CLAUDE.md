...
- **编码场景默认路由**：涉及代码开发、调试、重构、审查、构建、验证、提交、AI 编码工具/skill 选型时，默认先调用 `ai-coding-guide` 定分类与下游 skill；用户明确点名其他 skill/命令则尊重用户指令。
- **写作场景默认路由**：涉及写、改、审、润色、排版、查重、配图、发布前检查中文技术文章/系统教程时，默认先调用 `article-writing-guide` 定阶段与下游 skill；用户明确点名其他 skill/命令则尊重用户指令。
- **学习场景默认路由**：涉及学习调研、课程/教程制作、从零系统学习某主题时，默认先调用 `learning-guide`。
- **前端/视觉场景默认路由**：涉及 UI、页面、组件、落地页、视觉设计、动画动效时，默认先调用 `frontend-guide`。
- 多域任务（如「学前端顺便写代码」）逐个域调用对应路由，不塞进编码路由。
...
2. **数据结构选型**：底层结构选对否？位宽是否按需升级？dict vs map vs 数组取舍有无数据支撑？**无 benchmark 不下选型结论**——按 §11 证据门槛执行。
...
**权限归属（按 §6）：** 单 reviewer = 询问确认；三 agent 对抗 = 必须确认。
...
- 多步任务每轮重述进度（「第 3/5 步完成：X。下一步：Y」），不靠用户跨轮记。
- >1 步的任务用编号列表，每步一个可执行动作，不堆散文步骤。
- 列表≤5 项；超 5 拆「现在/稍后」或「必须/可选」。
- 时间估计给具体单位（「约 15 分钟」/「半天」），不写「一会儿/有些工作」。
- 错误写原因+修法，不用「糟糕/哎呀/似乎有问题」。
