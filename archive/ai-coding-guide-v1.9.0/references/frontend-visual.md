...
本文件只在 Step 1 命中「前端视觉」分类时展开。主文件 `SKILL.md` 只留决策骨架一行；本文件存完整五分支 / AskUserQuestion / 组合顺序 / 验证。
...
需要浏览器行为、可访问性或视觉证据 → 实现后串联 `browser-testing-with-devtools`（chrome-devtools MCP 本机已配置），不另建 router。
